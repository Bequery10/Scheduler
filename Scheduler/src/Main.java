...
public class Main {
	...
	private static List<List<List<String>>> pCHelper(List<Boolean> stack1, int count, List<String> nameList,
			List<List<String>> combination, List<List<List<String>>> combinations, boolean[][] occupiedHours) {
		...
		for (String s : table.keySet()) { // ----------class
			...
			for (Integer i1 : classTable.keySet()) { // ---------------section
				...
				for (String s1 : classTable.get(i1).keySet()) { // --------------------- day-hours
					...
					if ((hours.get(0) - 1) <= earlyHour)
						earlyHours1++;

					if ((hours.get(0) - 1) >= 10)
						combinations1.add(day + "  " + (hours.get(0) - 1) + " - " + hours.get(hours.size() - 1));
					else
						combinations1.add(day + "  " + "0" + (hours.get(0) - 1) + " - " + hours.get(hours.size() - 1));
					...
					if (count2 == classTable.get(i1).size()) {
						...
						combinations = pCHelper(stack2, count, nameList, combination, combinations,
								occupiedHoursDataSaver);
